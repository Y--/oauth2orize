...
module.exports = function code(options, issue) {
  ...
  function response(txn, res, next) {
    ...
    try {
      ...
      var context = { session : res.req.session };
      if (arity === 6) {
        issue(txn.client, txn.req.redirectURI, txn.user, txn.res, context, issued);
      } else { // arity == 5
        issue(txn.client, txn.req.redirectURI, txn.user, context, issued);
      }
    } catch (ex) {
      return next(ex);
    }
