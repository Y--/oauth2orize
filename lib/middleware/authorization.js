...
module.exports = function(server, options, validate, immediate) {
  ...
  options = options || {};
  immediate = immediate || function (client, user, context, done) { return done(null, false); };
  ...
  return function authorization(req, res, next) {
    ...
    server._parse(type, req, function(err, areq) {
      ...
      function validated(err, client, redirectURI) {
        ...
        if (arity === 5) {
          immediate(req.oauth2.client, req.oauth2.user, req.oauth2.req.scope, context, immediated);
        } else { // arity == 4
          immediate(req.oauth2.client, req.oauth2.user, context, immediated);
        }
      }
      ...
      try {
        ...
        if (arity === 4) {
          validate(areq.clientID, areq.redirectURI, context, validated);
        } else if (arity === 5) {
          validate(areq.clientID, areq.redirectURI, areq.scope, context, validated);
        } else if (arity === 6) {
          validate(areq.clientID, areq.redirectURI, areq.scope, areq.type, context, validated);
        } else { // arity == 3
          validate(areq, context, validated);
        }
      } catch (ex) {
        return next(ex);
      }
