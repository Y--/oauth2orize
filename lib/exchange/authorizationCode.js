...
module.exports = function(options, issue) {
  ...
  return function authorization_code(req, res, next) {
    ...
    // Some clients put it in the query.
    code = code || req.query.code;
    redirectURI = redirectURI || req.query.redirect_uri;
    if (!code) { return next(new TokenError('Missing required parameter: code', 'invalid_request')); }

    try {
      issue(client, code, redirectURI, context, function(err, accessToken, refreshToken, params) {
